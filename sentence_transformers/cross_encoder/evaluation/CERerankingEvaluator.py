...
class CERerankingEvaluator:
    ...
    """
    def __init__(self, samples, mrr_at_k: int = 1, name: str = '', write_csv: bool = True):
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        for instance in self.samples:
            ...
            #-------------------
            pred_scores2 = torch.tensor(pred_scores).unsqueeze(0)
            pred_scores2 = 10 - torch.tensor(pred_scores2)
            _, indices = torch.sort(pred_scores2, dim=1)
            _, indices = torch.sort(indices, dim=1)

            result = indices.cpu().numpy().tolist()
            if result[0][0] == 0:
                p1 += 1
            MRR += (1.0 / (result[0][0] + 1))
            #-------------------
