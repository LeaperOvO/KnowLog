...
class SentenceTransformer(nn.Sequential):
    ...
    # enhance
    def smart_batching_collate_enhance(self, batch):
        num_texts = len(batch[0].texts)
        texts = [[] for _ in range(num_texts)]
        labels = []
        ...
        for idx in range(num_texts):
            ...
            l = torch.ones(labels_mlm.shape, dtype=torch.long)
            l = torch.where(attention_mask == 0, 0, l)
            l = torch.where(input_ids == self._first_module().tokenizer.get_vocab()['[SEP]'], 0, l)
            l = torch.where(input_ids == self._first_module().tokenizer.get_vocab()['[CLS]'], 0, l)
            special_tokens_mask = l.bool()
            probability_matrix.masked_fill_(~special_tokens_mask, value=0.0)
            masked_indices = torch.bernoulli(probability_matrix).bool()
            labels_mlm[~masked_indices] = -100
            input_ids[masked_indices] = self._first_module().tokenizer.convert_tokens_to_ids("[MASK]")
            tokenized["input_ids"] = input_ids
            label_dict["mlm_label"] = labels_mlm
            sentence_features.append(tokenized)
        return sentence_features, label_dict



    # lianggang
    def smart_batching_collate_token(self, batch):
        num_texts = len(batch[0].texts)
        texts = [[] for _ in range(num_texts)]
        labels = []
        ...
        aa = sentence_features[0]['input_ids']
        new_label = torch.where(aa != 103, -100, aa)
        labels = labels.unsqueeze(1).repeat((1, new_label.size(1)))
        new_label = torch.where(aa == 103, labels, new_label)

        return sentence_features, new_label

    def smart_batching_collate2(self, batch):
        num_texts = len(batch[0].texts)
        texts = [[] for _ in range(num_texts)]
        labels = []
        new_input = []
        ...
        for i in range(len(texts[0])):
            new_input.append(tuple([texts[0][i],texts[1][i]]))
        ...
        tokenized = self.tokenize(new_input)
        sentence_features.append(tokenized)

        return sentence_features, labels


    def smart_batching_collate_embedding(self, batch):
        """
        Transforms a batch from a SmartBatchingDataset to a batch of tensors for the model
        Here, batch is a list of tuples: [(tokens, label), ...]
        ...
        embedd = torch.tensor([item.cpu().detach().numpy() for item in embedding])
        ...
        return sentence_features, embedd

    def multi_batching_collate(self, batch, padding=False):
        ...
        sentence_features = []
        for idx in range(num_texts):
            tokenized = self.tokenize(texts[idx], padding=False)
            sentence_features.append(tokenized)
    ...
    def fit(self,
            ...
            evaluator2: SentenceEvaluator = None,
            epochs: int = 1,
        ...
        if save_f:
            ...
            dataloaders[0].collate_fn = self.smart_batching_collate_embedding
            if len(dataloaders) == 2:
                dataloaders[1].collate_fn = self.smart_batching_collate
        else:
            for dataloader in dataloaders:
                dataloader.collate_fn = self.smart_batching_collate
                # dataloader.collate_fn = self.smart_batching_collate_token

        # train with Enhance 注意将label to gpu改回来
        # dataloaders[0].collate_fn = self.smart_batching_collate_enhance
        # dataloaders[1].collate_fn = self.smart_batching_collate
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            self._eval_during_training2(evaluator2, output_path, save_best_model, epoch, -1, callback)

            # if epoch % 10 == 0:
            #     self.save(output_path + '_' + str(epoch))

            if save_f and epoch % 10 == 0:
                self.save(output_path + '_' + str(epoch))
    ...
    def _eval_during_training2(self, evaluator, output_path, save_best_model, epoch, steps, callback):
        """Runs evaluation during the training"""
        eval_path = output_path
        if output_path is not None:
            os.makedirs(output_path, exist_ok=True)
