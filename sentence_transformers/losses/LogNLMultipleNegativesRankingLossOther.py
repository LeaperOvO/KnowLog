...
class LogNLMultipleNegativesRankingLossOther(nn.Module):
    """
    ...
    def __init__(self, model: SentenceTransformer, scale: float = 20.0, similarity_fct = util.cos_sim):
        ...
        """
        super(LogNLMultipleNegativesRankingLossOther, self).__init__()
        self.model = model
        self.scale = scale
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor):
        ...
        embeddings_b = labels
        embeddings_c = self.model(sentence_features[1])['sentence_embedding']
        ...
        return 0.5 * self.cross_entropy_loss(scores, labels) + 0.5 * self.cross_entropy_loss(scores2, labels2)
